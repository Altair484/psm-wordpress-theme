 ...
 */
function activate_custom_login_page()
{
	register_setting('general', 'activate_custom_login_page', 'esc_attr');
	add_settings_field('activate_custom_login_page', '<label for="custom_login_page">'.__('Activer la page de connexion personalisée' , 'activate_custom_login_page' ).'</label>' , 'show_custom_login_activate_field', 'general', 'login_section');
}
...
function show_custom_login_activate_field()
{
	...
	if (!get_option( 'activate_custom_login_page')){
		add_option( 'activate_custom_login_page', 'true');
	}

	//If the option is empty, we set it's value to "false"
	if(get_option('activate_custom_login_page') == ''){
		update_option('activate_custom_login_page', 'false');
	};

	$value = get_option( 'activate_custom_login_page');?>
	<input type="checkbox" id="activate_custom_login_page" name="activate_custom_login_page" <?php $value == 'true' ? _e('checked') : _e('') ?> value="true" />
...
<?php }
add_filter('admin_init', 'activate_custom_login_page');
